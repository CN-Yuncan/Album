      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.configs.createMany({
          data: [
            ...
            { config_key: 'cos_region', config_value: '', detail: '腾讯云 COS Region 地域，如：ap-guangzhou' },
            { config_key: 'cos_bucket', config_value: '', detail: '腾讯云 COS Bucket 存储桶名称' },
            { config_key: 'cos_storage_folder', config_value: '', detail: '存储文件夹(COS)，严格格式，如：picimpact 或 picimpact/images ，填 / 或者不填表示根路径' },
            { config_key: 'cos_cdn', config_value: 'false', detail: '是否启用 COS CDN 模式，路径将返回 cdn 地址，默认 false。' },
            { config_key: 'cos_cdn_url', config_value: '', detail: 'cdn 地址，如：https://cdn.example.com' },
